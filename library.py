...
class GraphOfWordsUtils:
    def convert_terms_to_graph(self, terms, w):
    ...
    def get_k_core_keywords(self, graph, weighted, minimum_k=None):
        core_numbers = self.decompose(graph, weighted)
        if minimum_k is None:
            threshold_k = max(core_numbers.values())
        else:
            threshold_k = minimum_k
        # retain main core as keywords
        keywords = [kwd for kwd, c_n in core_numbers.items() if c_n >= threshold_k]
        return keywords

    def get_main_core_keywords(self, graph, weighted):
        return self.get_k_core_keywords(graph, weighted)
